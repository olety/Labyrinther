...
class Moveset:
    ...
    def __init__(self, num_moves=0, string=None, move_list=None):
        if num_moves:
            str_arr = np.array(
                ["{0:02b}".format(random.getrandbits(2)) for i in range(0, num_moves)]
            )
    ...
    def _parse_move_string(self, string):
        ...
        self.move_list = np.array(
            [
                Move.from_string(character + next(iterator_str, ""))
                for character in iterator_str
            ]
        )
    ...
    def move_string(self):
        return "".join(
            "{0:02b}".format(move.direction.value) for move in self.move_list
        )
    ...
    def crossover(self, parent2, num_points):
        logging.debug(
            "Moveset - Crossover - self len = {}, p2 len = {}, num pts = {}".format(
                len(self), len(parent2), num_points
            )
        )
        if num_points < 0 or num_points > len(self):
            raise Exception(
                "Moveset - bad number of points for crossover = {}".format(num_points)
            )
        ...
        crossover_points = sorted(
            [random.randint(0, rand_len) for i in range(num_points)]
        )
        current_parent = 0
        current_point = 0
        for point in crossover_points:
            child = np.concatenate(
                [child, parents[current_parent][current_point : point + 1]]
            )
