...
class Labyrinth:
    def __init__(self, rows=5, cols=5, file=None, file_obj=None):
        ...
        logging.debug("Labyrinth - Shape: [{},{}], file={}".format(rows, cols, file))
        if file_obj:
        ...
        self.num_moves_max = (
            self.array_closed.shape[0] * self.array_closed.shape[1] / 100
        )
    ...
    def _from_file(self, file):
        with open(file, "r") as f:
            try:
                ...
                raise Exception("Bad input labyrinth file")
            logging.debug(
                "Labyrinth - file - Found shape: [{},{}]".format(self.rows, self.cols)
            )
        temp_df = pd.read_csv(file, sep="-", header=None, skiprows=1)
        self.array_closed = temp_df.values
        # print(self.array_closed)
    ...
    def _next_cell(self, row, col):
        ...
        while next_path is False:
            ...
            if len(choices) < 1:
                logging.debug(
                    "Labyrinth - generator - Stopping due to wall encounter, stepping back"
                )
                return False
            # Choose a direction
            ...
            if chosen_dir == Direction.TOP:
                ...
                next_col += 1
            logging.debug(
                "Labyrinth - init - at node [{},{}], going {} -> next [{},{}]".format(
                    row, col, chosen_dir.name, next_row, next_col
                )
            )
    ...
    def save_file(self, name):
        ...
        with open(name + ".csv", "a") as f:
            pd.DataFrame(data=self.array_closed.astype(int)).to_csv(
    ...
    def _plot_moveset(self, path, bumps, **kwargs):
        ...
        if kwargs.get("plot", True):
            ...
            for i in range(len(path) - 2, -1, -1):
                p = FancyArrowPatch(
                    posA=[path_xs[i], path_ys[i]],
                    posB=[path_xs[i + 1], path_ys[i + 1]],
                    connectionstyle="arc3, rad=0.5",
                    arrowstyle="simple, head_width={}, head_length={}".format(
                        30 - (self.rows + self.cols), 25 - (self.rows + self.cols)
                    ),
                    edgecolor="#5f5d63",
                    facecolor="#42c8f4",
                    zorder=2 + i,
                ...
                plots.append(
                    ax.scatter(
                        path_xs[i + 1], path_ys[i + 1], zorder=2 + i, color="#42c8f4"
                    )
                )
                ax.add_artist(p)
                plots.append(p)
    ...
    def _arr_to_cells(array, rows, cols, open_exit=False):
        logging.debug(
            "Labyrinth - Converting array [{},{}] to cells. Arr shape: [{}]".format(
                rows, cols, array.shape
            )
        )
        multiplier = array.shape[1] // rows
        logging.debug(
            "Labyrinth - array conversion multiplier is {}".format(multiplier)
        )
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if (
                    row > 0
                    and array[row * multiplier, col * multiplier - 1 + multiplier // 2]
                    == 0
                ):
                ...
                if (
                    row < rows - 1
                    and array[
                        (row + 1) * multiplier, col * multiplier - 1 + multiplier // 2
                    ]
                    == 0
                ):
                ...
                if (
                    col > 0
                    and array[row * multiplier - 1 + multiplier // 2, col * multiplier]
                    == 0
                ):
                ...
                if (
                    col < cols - 1
                    and array[
                        row * multiplier - 1 + multiplier // 2, (col + 1) * multiplier
                    ]
                    == 0
                ):
    ...
    def __str__(self):
        return "{}x{} Labyrinth. {}x multiplier is used in a file representation. num_moves_max = {}".format(
            self.rows, self.cols, 10, self.num_moves_max
        )
    ...
    def _cells_to_arr(
        movecells,
        open_endpoints=False,
        multiplier=10,
        line_thickness_hor=1,
        line_thickness_ver=1,
    ):
        ...
        for ver in range(movecells.shape[0]):
            for hor in range(movecells.shape[1]):
                ...
                if movecells[ver, hor, Direction.TOP] == 1:
                    dup[
                        ver_start - line_thickness_ver : ver_start + cell_height,
                        hor_start : hor_start + cell_width,
                    ] = 0
                if movecells[ver, hor, Direction.BOTTOM] == 1:
                    dup[
                        ver_start : ver_start + cell_height + line_thickness_ver,
                        hor_start : hor_start + cell_width,
                    ] = 0
                if movecells[ver, hor, Direction.LEFT] == 1:
                    dup[
                        ver_start : ver_start + cell_height,
                        hor_start - line_thickness_hor : hor_start + cell_width,
                    ] = 0
                if movecells[ver, hor, Direction.RIGHT] == 1:
                    dup[
                        ver_start : ver_start + cell_height,
                        hor_start : hor_start + cell_width + line_thickness_hor,
                    ] = 0
        return dup
